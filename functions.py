...
def print_sign(num):
    """Print if number is higher, lower, or equal to zero"""
    if num>0:
        print("Higher than 0")
    elif num<0:
        print("Lower than 0")
...
def is_divisible_by_three(num):
    """Return True of False depending on whether number is evenly divisible by 3"""
    #Use the mod operator to check remainder, and return true if it is 0, false otherwise.
    if num%3 == 0:
        return True
    else:
        return False
...
def total_meal_price(meal_price, tip_perc=0.15):

    return (meal_price + (meal_price*tip_perc))
...
def write_letter(recipient, job, sender):
    """Prints out letter to recipient with their job name, and sender name"""
    #Call full_title(name, job) to construct the first line of the letter

    header_line = full_title(recipient,job)

    print(f'Dear {header_line}, I think you are amazing! Sincerely, {sender}')
